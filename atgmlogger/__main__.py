...
from . import (__description__, __version__, LOG_LVLMAP, LOG_FMT, DATE_FMT,
               TRACE_LOG_FMT)
...
def parse_args(argv=None):
    ...
    parser = argparse.ArgumentParser(prog='ATGMLogger',
    ...
    sub = parser.add_subparsers(dest='command',
                                help="Subcommands to run/install/uninstall "
                                     "ATGMLogger")

    install_parser = sub.add_parser('install',
                                    help='Install system files for ATGMLogger',
                                    allow_abbrev=True)
    install_parser.add_argument('--service', action='store_true', default=True,
                                help='Install ATGMLogger as a Systemd Service')
    install_parser.add_argument('--dependencies', action='store_true',
                                help='Install system dependencies using apt.')
    install_parser.add_argument('--configure', action='store_true',
                                help='Run RaspberryPi configuration scripts.')
    install_parser.add_argument('--check-install', action='store_true',
                                help='Verify installed components.')
    install_parser.add_argument('--logrotate', action='store_true',
                                default=True, help='Install logrotate '
                                                   'configuration')
    install_parser.add_argument('--with-mqtt', action='store_true',
                                help='Install MQTT plugin configuration and AWS'
                                     ' IoT dependencies')

    uninst_parser = sub.add_parser('uninstall',
                                   help='Uninstall ATGMLogger system files and '
                                        'configurations')
    uninst_parser.add_argument('--keep-config', action='store_true',
                               help='Retain configuration after uninstalling '
                                    'ATGMLogger')

    chkinst_parser = sub.add_parser('chkinstall',
                                    help='Check ATGMLogger installation and '
                                         'depdendencies')

    run_parser = sub.add_parser('run', help='Run atgmlogger')
    ...
    if not logdir.exists():
        try:
            ...
            LOG.warning("Log directory could not be created, log "
                        "files will be created in current directory (%s).",
                        str(Path().resolve()))
            logdir = Path()
...
def initialize(args):
    """Initialize global application params and/or execute install methods"""
    if args.debug:
    ...
    if args.command in {'install', 'uninstall', 'chkinstall'}:
        ...
        if method is None:
            LOG.error("Command %s is not recognized or implemented.",
                      args.command)
            sys.exit(1)
...
def entry_point():
    ...
    sys.exit(atgmlogger(verbosity=args.verbose))
